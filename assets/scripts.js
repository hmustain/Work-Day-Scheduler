...
// created global var for userInput tutor helped me on this one 
var userInput;
...
if (storedInputs) {
  ...
  hour8Input.value = storedInputs["hour-8"];
  hour9Input.value = storedInputs["hour-9"];
  hour10Input.value = storedInputs["hour-10"];
  hour11Input.value = storedInputs["hour-11"];
  hour12Input.value = storedInputs["hour-12"];
  hour13Input.value = storedInputs["hour-13"];
  hour14Input.value = storedInputs["hour-14"];
  hour15Input.value = storedInputs["hour-15"];
  hour16Input.value = storedInputs["hour-16"];
  hour17Input.value = storedInputs["hour-17"];
}
...
for (i of enterBtn) {
  i.addEventListener("click", function (event) {
    ...
    userInput = {
      "hour-8": hour8Input.value.trim(),
      "hour-9": hour9Input.value.trim(),
      "hour-10": hour10Input.value.trim(),
      "hour-11": hour11Input.value.trim(),
      "hour-12": hour12Input.value.trim(),
      "hour-13": hour13Input.value.trim(),
      "hour-14": hour14Input.value.trim(),
      "hour-15": hour15Input.value.trim(),
      "hour-16": hour16Input.value.trim(),
      "hour-17": hour17Input.value.trim(),
    };
...
for (i of eraseBtn) {
  i.addEventListener("click", function (event) {
    ...
    // tutor helped me with my delete button to only delete what i wanted to delete instead of the whole thing
    if (localStorage.getItem("user input")) {
      $(this).siblings("input").val("");
      console.log("testing", $(this).siblings("input").attr("id"));
